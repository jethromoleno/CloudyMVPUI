...
const Sidebar: React.FC<SidebarProps> = ({ 
  currentView, setCurrentView, onLogout, username, userRole, theme, onSelectModule 
}) => {
        ...
        >
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 bg-navy-900 dark:bg-white rounded-lg flex items-center justify-center shadow-lg shadow-navy-900/20 dark:shadow-none">
              <Map className="text-white dark:text-carbon-900 w-4 h-4" />
            </div>
            <div className="text-left">
              <p className="text-[10px] font-bold text-navy-400 dark:text-carbon-500 uppercase tracking-widest">Workspace</p>
              <p className="text-sm font-bold text-navy-900 dark:text-white truncate">LogiTrack AI</p>
            </div>
          </div>
              ...
              {apps.map((app) => (
                <button
                  ...
                  className={`w-full flex items-center gap-3 p-3 rounded-lg transition-colors ${
                    app.id === 'trip_scheduling' 
                    ? 'bg-navy-50 dark:bg-carbon-800 text-navy-900 dark:text-white' 
